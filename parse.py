    ...
    def _get_subcat_lvl2_objects(self):
        for subcat_lvl1_obj, s_lvl1_name, cat_name  in self._get_subcat_lvl1_objects():
            ...
            for obj in objs:
                ...
                p_name = s_lvl1_name
                g_p_name = cat_name
                yield obj, name, p_name, g_p_name, url

    def get_lvl2_subcategories(self):
        for _, name, p_name, g_p_name, url in self._get_subcat_lvl2_objects():
            yield name, p_name, g_p_name, url

    def _get_subpage_urls(self, subcat_lvl2_url):
        soup = self.get_soup(subcat_lvl2_url)
        ...
        for name, p_name, g_p_name, url in self.get_lvl2_subcategories():
            for subpage_url in self._get_subpage_urls(url):
                for url in self._extract_product_link(subpage_url):
                    p_name_ = name
                    g_p_name_ = p_name
                    g_g_p_name_ = g_p_name
                    yield url, p_name_, g_p_name_, g_g_p_name_
